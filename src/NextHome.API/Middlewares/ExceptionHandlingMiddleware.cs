...
namespace NextHome.API.Middlewares
{
    public class ExceptionHandlingMiddleware
    {
        ...
        public async Task Invoke(HttpContext context)
        {
            try
            {
                await _next(context); // Continua a execução normalmente
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "An unhandled exception was caught by the middleware.");

                // Garante que a exceção não continue se propagando
        ...
        private static async Task HandleExceptionAsync(HttpContext context, Exception exception)
        {
            ...
            {
                return; // Evita tentar modificar uma resposta já iniciada
            }
